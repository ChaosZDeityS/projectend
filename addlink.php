...
<?php
    ...
    require 'connect.php' ; 
    $sql =" TRUNCATE TABLE temp_linkbuslineandbusstop ";
...
?>
  ...
  <body id="page-top">
      ...
      <div id="content-wrapper">
        ...
        <div class="container-fluid">
          ...
          <div class="card mb-3">
            ...
            <form method="GET" action = "addbuslink_insert.php" > 
             สายรถเมล์คันที่ :        <?php
     // 1. Connect
     require("connect.php");
   //  $selection = $_SESSION['selection']; 
     ...
     echo "<select name='buslineid' id='buslineid' >";
     echo "<option value='0'>".กรุณาเลือกสายรถเมล์."</option>";
     while($row = mysqli_fetch_assoc($result)) {
       if($row['buslineid'] == $buslineid ){
        echo " <option value='".$row['buslineid']."' selected>" .$row['buslineid']." : ".$row['buslinename']."</option>";
       }else{
   ...
   ?>
        ...
        <?php
                        ...
                        while($row = mysqli_fetch_assoc($result)) {
                         // $buslineid = $row['buslineid'];
                          $busstopnameTH = $row['busstopnameTH'];
                          $busstopid = $row['busstopid'];
                        
                          echo "<input type='checkbox' name='link[]' value='$busstopid' id='$busstopid' >";
                          echo "<label for='$busstopid'> &nbsp &nbsp &nbsp $count :&nbsp&nbsp$busstopnameTH</label><br/>";
                          $count++;
                        }
