...
$sql = "SELECT stamp.pie_id,COUNT(member.t_id ) nation
		FROM stamp
		INNER JOIN member
		ON member.t_id = stamp.t_id
		AND member.nation = 'THAILAND'
		 GROUP by pie_id";

$sql2 = "SELECT stamp.pie_id,COUNT(member.t_id ) nation
		FROM stamp
		INNER JOIN member
		ON member.t_id = stamp.t_id
		AND member.nation = 'FOREIGN'
		 GROUP by pie_id";
//จบ sql
...
<html>
 ...
 $(function () {
        $('#container').highcharts({
            ...
            title: {
                text: 'กราฟวิเคราะห์ระหว่างชิ้นงานกับสัญชาติ' //หัวข้อของกราฟ
            },
            subtitle: {
